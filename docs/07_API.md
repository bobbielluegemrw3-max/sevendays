...
- GET `/api/v1/horses/{id}` (2026-07-11 Decision 087 audit: response additionally carries `listing` and `history` — the horse's full race record from race_results/races: batch_date, final_rank/final_score, is_burned, participant_count and the revealed conditions weather/track_condition/surface; 2026-07-14: also `trained_for_next_race` — same effective-race-date rule as GET /horses — so the detail page can collapse the training form into a done state). POST `/horses/{id}/training` and POST `/horses/{id}/item` now reject a MANUALLY listed horse with HORSE_MARKET_LOCKED (409) — a market-locked horse does not race tonight, so training rights and item units must not be wasted on it.
...
- GET `/api/v1/daily-derby/status` (2026-07-12 spike hardening: the shared part — batch/race/counts/ticker/forecast — is process-cached ~2s via `DERBY_STATUS_CACHE_MS` because every show viewer polls at 5s; per-user data is one indexed my_horses query; the old always-computed-never-consumed `personal` block was removed and the field is now always null — personal night results live in `/daily-derby/my-results/:date`; date filters rewritten sargable with supporting indexes, migration 20260712130000. Decision 093 adds `tonight_field` {entrants, burn_slots_min, burn_slots_max}: ACTIVE horses minus live MANUAL listings, burn-slot bounds = floor(entrants x jitter envelope 8.0%..13.5%) — always a true bound regardless of policy/status)
- POST `/api/v1/horses/{id}/training` (Decision 066)
- POST `/api/v1/account/stable-name` (Decision 097): set/clear the public stable name (2-20 chars, unique case-insensitively, one change/day). GET /me gains stable_name. Shown on the my-stable title, matching counterparts, champions hall, gift senders and the org map (as 厩舎名(ab***@domain) — emails stay masked at every tier). Admin: POST `/admin/stable-name/clear` {user_id, reason} (audited moderation). Errors: STABLE_NAME_INVALID, STABLE_NAME_TAKEN, STABLE_NAME_DAILY_LIMIT.
- POST `/api/v1/promo/redeem` (Decision 095): redeem a seminar code (SDD-XXXX-XXXX) for one promo-stable horse (youngest first, gifted_at semantics). One per user per campaign; single-use codes with optional expiry. Admin side: POST/GET `/admin/promo/codes` (mint/list batches + stock summary), POST `/admin/promo/gift` (direct on-site distribution, audited, exempt from the 3/day transfer cap, one per recipient per day). Errors: PROMO_CODE_INVALID, PROMO_CODE_USED, PROMO_CODE_EXPIRED, PROMO_ALREADY_REDEEMED, PROMO_OUT_OF_STOCK.
- POST `/api/v1/horses/{id}/transfer` (Decision 094): gift an ACTIVE, unlisted horse to another user by email (in-game asset move; no user-to-user USDT exists). Marketplace must be OPEN. Sender cap 3 horses/24h; one transfer per horse per day (idempotency key horse-gift:{id}:{date} — also blocks same-day A->B->C chains). Sets horses.gifted_at permanently: gifted horses can never be MANUALLY listed (smart listing eligibility is unchanged so gifting cannot be used to dodge engine sales). Recipient gets HORSE_GIFT_RECEIVED. Errors: HORSE_NOT_FOUND, NOT_HORSE_OWNER, HORSE_NOT_ACTIVE, HORSE_LISTED, GIFT_RECIPIENT_NOT_FOUND, GIFT_SELF, HORSE_TRANSFER_LIMIT, HORSE_TRANSFER_DAILY, MARKETPLACE_LOCKED. POST /market/list gains HORSE_GIFTED_NO_MANUAL_LISTING.
...
Support Bonus (Decision 074, payout trigger amended by Decision 092 to champion celebrations; user-facing name サポートボーナス — never "MLM"):
...
- GET `/support/summary` returns the caller's referral_code, unlocked_tiers (1..7), volume (direct referrals' ACTIVE-horse current value), tier amounts/thresholds, pool_count and lifetime bonuses received. Decision 099 adds `starter_rate` — the caller's CURRENT tier-1 celebration amount, clamp(150000/org_volume, 3.00, 8.00); the amount actually paid is fixed on the champion night from the tier-1 ancestor's volume that night.
- GET `/support/pool` lists unplaced direct referrals (masked display names). GET `/support/network` returns the placement subtree down to tier 7 (max 500 nodes). GET `/support/bonuses` lists received payments (amount, tier, burn_event_id — since Decision 092 the reference id points at the celebration row; the field name stays for compatibility).
