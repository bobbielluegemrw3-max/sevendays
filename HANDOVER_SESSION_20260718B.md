...
- **-7c UI(ショー新幕)も完了 — V2実装は全フェーズ完了。次=テストネット試運転(§2)**
...
## 1.5 総合100点診断(2026-07-18・試運転前)— **92点 → 追修正で97点**

**追修正(オーナー指示「上げられる分は上げる」・4件)**:
①スタックユニット掃除 — post-batchスイープが「対象レース完了済みのPENDING使用」を
自動キャンセルして在庫へ復元(未来サイクルは対象外・V1の既知エッジも同時解消・テスト済み)
②ナビカウントダウンのV2対応 — 朝8:00/夜20:00の近い方へ(layout→TopNav→DerbyCountdownに
engineV2を配線・V1は20:00のみで不変) ③透明性サマリー120行(V2でも約60日分)
④自動プール通知を再実行でも常に試行(dedupe吸収・クラッシュ欠落の自己修復)
**残り3点(試運転が検証手段)**: ショー新幕の実機視覚QA(−2)・サイクル別アーカイブUI(−1)

- **診断で発見→修正済み(`599875b`)**: ①V2アイテム取消の複数行キャンセルで在庫復元が1個のみ
  (残りがAPPLIEDのまま宙に浮く)→全行復元に修正 ②透明性CSVの同日2行が区別不能→slot列追加
- **既知エッジ(未対応・試運転で監視)**:
  ①手動出品(Market Lock)でレースをスキップした馬のPENDINGアイテム使用は残留し得る
  (ユニット+代金がITEM_CLEARINGに滞留 — V1由来の既知エッジがTRAINING種にも拡大。
  頻度低・対策=残留PENDING掃除ジョブを試運転後に検討)
  ②ナビのカウントダウン(`secondsToNextDerbyLocal`)は20:00のみ — 朝レース非対応(-7残)
  ③透明性サマリー60行制限=V2では約30日分に半減 ④日別アーカイブ/台帳=同日2レースは
  最新(夜)優先の暫定表示 ⑤自動プール通知はcreate後クラッシュで欠落し得る(ベストエフォート)
- **視覚QA未**: ショー新幕(YOUR NEW STABLE/JP)の通し実機確認は試運転初週に
- 冪等性・クラッシュ窓・V1不変性・commit-reveal検証可能性・会計整合は全経路で確認済み
  (詳細は当該セッションの診断レポート参照)
