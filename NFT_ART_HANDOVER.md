...
セッション終了時点で以下が**未コミット**(オーナーが localhost:3000 で最終確認 → OKならコミット&プッシュが次の一手):
...
**コミット時の要点**(コミットはオーナー指示後): 上記全部+この引継ぎ書を1コミットで。メッセージ例:
`web: 3-archetype NFT look engine (Manus full-colour layers x approved hue variants)`
...
1. **オーナー最終確認 → コミット&プッシュ → 本番確認**(sevendaysderby.com)
