...
### 夜の追加セッション(23時〜): 初本番ライブの障害調査 → テストネット全リセット

- **症状**: 朝に3アカウント(goldbenchan/kusano/guri)へ1,000 USDT付与→購入予約8件→20:00バッチは37ステップ正常完走したのに馬が誕生しなかった
- **原因(バグではない)**: **ミント担保ゲート(Decision 069)が正しく作動**。過去のテスト馬2頭(DAY7_CLEARED)のチャンピオン報酬残債285.71 USDT > 買戻準備金74.91(Decision 089でfund-grant原資を入金クリアリングにしたため準備金に収入がなかった)→ Day0ミント枠0 → 8件全て未割当→全額返金(EXPIRED)。さらに放置すると7/15にPAY_DUE_BUYBACKSがCOMMIT時失敗(負残高禁止トリガー)で**バッチ全体が停止**する状態だった
- **対処(オーナーGO)**: `20260713030000_testnet_reset.sql`(`aa1653b`)で**ゲーム/経済データ全消去**(TRUNCATEは行トリガーの不変性ガードを発火させない・TRUNCATEトリガー不存在確認済み)。**残したもの**: users 11行(運営ルートチェーン=usersのカラムなので配置ツリー無傷)・admin_role_grants・user_trade_settings・push・CS・item_catalog・全ポリシー・ledger_accounts。消去前に全891行をscratchpadへJSONバックアップ
- **副作用(想定内)**: batch_runsを消したためワーカーの自己修復が今夜分を空データで即再実行→正常完走(ローンチ初日シナリオ)。今日の行ができたので再発火なし
- **残タスク**: オーナーが管理画面から3アカウントへ1,000 USDTを再付与(Decision 089の即時付与はちょうど1,000まで可)→ 各自購入予約 → 明晩(7/14)20:00にDay0誕生。**kusano/guriの旧予約は消えているので再予約必須**
- **教訓**: 少人数テストでは馬がDAY7クリアするたび同じゲートが働き得る(手数料収入が報酬約束に追いつかない)。その場合は準備金へのテスト資金振替1発で解決(慌てない)。本番ローンチは既存債務ゼロから始まるので影響なし
