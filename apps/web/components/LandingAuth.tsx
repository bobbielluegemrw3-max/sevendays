...
/**
 * MetaMask + Google login buttons for the landing CTA (Decision 071).
 * Real sign-in — same flow as the login page. Layout is faithful to the
 * design handoff: MetaMask (cyan fill) →「または」→ Google (dark), stacked.
 */
export function LandingAuth() {
  ...
  return (
    <div className={s.authStack}>
      ...
      <div className={s.authSep}>
        <span className={s.ln} />
        または
        <span className={s.ln} />
      </div>
      <button className={`${s.authBtn} ${s.authGoogle}`} onClick={() => void google()} disabled={busy}>
