...
/* ============================================================================
 * PrepareV3 — 馬個別ページ V3 の「備える」(③今夜バー / C 調教 / D レースアイテム)。
 * ★ハンドオフ正典(/dev/horse-page-preview)の C/D を忠実に移植(構造・インラインstyle・
 *   状態機械・min-height・差し替え自由・装着演出まで一致)。本番 HorseDetailView(engine_v3)
 *   から使う。予報は群ラベル({axis,val})を受け取る。
 * ※ +デルタ(確定+3/装着+1/装備+2)は演出用の代表値。実バックエンド結線はV3起動時。
 * ========================================================================== */

/** CSSテキスト(正典からの写経)→ React style。 */
...
export function PrepareV3({ forecast, baseTotal }: { forecast: { axis: string; val: string }[]; baseTotal: number }) {
  ...
  // --border-soft / --good-dim は globals に無いのでここで補完(正典の値)。
  const rootVars = { '--border-soft': 'rgba(255,255,255,.055)', '--good-dim': 'rgba(53,208,127,.5)' } as unknown as CSSProperties;

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, ...rootVars }}>
      {/* ③ 今夜の条件ドライバーバー */}
