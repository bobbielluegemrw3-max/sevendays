...
describe('レースアイテム = 加算方式(§14.6)', () => {
  ...
  it('unknown / training keys contribute 0 to condition_prep', () => {
    ...
    expect(raceItemEdgeV4('golden_feed', ALL_CONDITIONS[0]!)).toBe(0); // TRAINING は prep に効かない
  });
...
describe('聖杯ラダーの throttle(§15) — total_value の伸びが+4/走に収まる', () => {
  ...
  it('drops the §14.5 draft top tiers (黄金+4〜6/秘伝+3〜5) that breach the 90 ceiling', () => {
    // 黄金は +3〜4 に、秘伝は +2.5〜3.5 に下方修正済み(§14.7 の p99≤88 制約)
    const golden = ITEM_BY_KEY_V4.get('golden_feed')!.effect as TrainingEffectV4;
    const secret = ITEM_BY_KEY_V4.get('secret_feed')!.effect as TrainingEffectV4;
