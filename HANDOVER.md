...
- 本番Supabase(project ref `bdljkptqmnewkjoqzviy`, region ap-south-1)に**マイグレーション31本適用済み・同期済み**。
...
- **★Daily Derbyライブ演出プロトタイプ(2026-07-06, Decision 073)**: 20:00バッチを「THE DAILY DERBY」約100秒のライブショーとして演出。3分前7セグSVGカウントダウン(残り30秒赤)→ファンファーレ16.8s(`public/sounds/`、原本WAVは未コミット`音声ファイル/`)→蹄音ループ(T+17-30s)→端末風高速ログ濁流(BURN赤/生存緑/価値/DAY7金→P2P出品/入札/マッチング/Day0→MLM/バフ配布)→TODAY RACE END→NFTアート入り個人結果。**絵文字禁止**(固定幅タグ+色ティック)。実装=`apps/web/lib/daily-derby.ts`(純タイムライン+決定論ログ生成)+`components/daily-derby/`。**プロトタイプを/racesに暫定マウント中(オーナー反復確認用・フィクスチャ駆動・シミュレーション操作パネル付き)**、視覚QAは`/dev/derby-preview`(本番404、`?t=秒&paused=1`で任意瞬間へ)。**残作業と再開手順の正典は `DAILY_DERBY_HANDOVER.md`**(バッチ状態API→実結線→ナビ共通カウントダウン等)。関連: 元ネタADRは`GPTとの会話/`(全ADR消化済み: MLM=074/077・マケプレ=076・buyback改名=075・アイテム=078/079)
- **★サポートボーナス実装完了(2026-07-07, Decision 074)**: 配置型7ティア(T1=3/T2=2/T3-7=1 USDT)。紹介リンク(`?ref=`→cookie→プロビジョニング固定・`apps/web/proxy.ts`)/未配置プール→1回限り配置(`/support`ページ・強警告ダイアログ)/Step16は配置ツリー遡り+ボリューム型ティア判定(`burn/support-bonus.ts`)/未達分は準備金滞留。API=`/api/v1/support/*` 5本+admin replace(SUPER_ADMIN・監査)。**呼称は「サポートボーナス」固定(MLM/紹介報酬と言わない・R3)**。仕様正典=`MLM_REVISION.md`・法務メモ=`LEGAL_REVIEW_MEMO.md`(弁護士レビューはオーナー側タスク)。視覚QA=`/dev/support-preview`。**ティア解放は2026-07-07にDecision 077で組織ボリューム化**(配下7段: 1万/2万/5万/25万/40万/60万+T5以上は直接紹介値を併用・根拠=`SUPPORT_TIER_SIMULATION.md`)
- **★2026-07-07の大規模実装(詳細記録=`WORKLOG_2026-07-06_07.md`)**: ①**見えるマーケットプレイス+手動出品(Decision 076)**=/market・Market Lock・正典`MARKETPLACE_REVISION.md` ②**チャンピオン報酬改名(075)**=/champion ③**Daily Derby音響完成**=iOS priming・8音源・自分該当行ハイライト+効果音(R4完了、残りR1〜R3) ④**アイテムシステム(078/079)**=販売30種+Burnドロップ5種・設定1〜6(シードcommit-reveal)・Burn時アイテム代→サポート財源・メール宛ギフト(`user_transfers`=将来のサイト内USDT送付の土台)・Manus画像35枚組込済み・正典`ITEM_REVISION.md`・画像発注書`ITEM_ART_PROMPT.md`(原本マスター=未コミット`seven_days_derby_items35.zip`)
...
2. **オーナー決定** → `docs/10_DECISION_LOG.md` に決定番号(次は**081**)で英語追記 + `IMPLEMENTATION_PLAN.md` 付録Eを更新
