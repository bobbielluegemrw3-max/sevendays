...
- **★Daily Derbyライブ演出プロトタイプ(2026-07-06, Decision 073)**: 20:00バッチを「THE DAILY DERBY」約100秒のライブショーとして演出。3分前7セグSVGカウントダウン(残り30秒赤)→ファンファーレ16.8s(`public/sounds/`、原本WAVは未コミット`音声ファイル/`)→蹄音ループ(T+17-30s)→端末風高速ログ濁流(BURN赤/生存緑/価値/DAY7金→P2P出品/入札/マッチング/Day0→MLM/バフ配布)→TODAY RACE END→NFTアート入り個人結果。**絵文字禁止**(固定幅タグ+色ティック)。実装=`apps/web/lib/daily-derby.ts`(純タイムライン+決定論ログ生成)+`components/daily-derby/`。**プロトタイプを/racesに暫定マウント中(オーナー反復確認用・フィクスチャ駆動・シミュレーション操作パネル付き)**、視覚QAは`/dev/derby-preview`(本番404、`?t=秒&paused=1`で任意瞬間へ)。**残作業と再開手順の正典は `DAILY_DERBY_HANDOVER.md`**(バッチ状態API→実結線→ナビ共通カウントダウン等)。関連: 元ネタADRは`GPTとの会話/`(MLM改定=オーナー詳細待ち・手動マケプレ=法的議論中・アイテムADR-009=次段)
