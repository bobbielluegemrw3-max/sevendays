...
| # | 残パラメータ | 期限 |
|---|---|---|
| P1 | Weather 4種・Track各状態の出現確率(均等25%か重み付けか)、Track状態の名称 | Phase 5 着手前 |
| P2 | condition/fatigue の数値パラメータ(training_cost、recovery量、Day0初期値) | Phase 5 着手前 |
| P3 | Bloodline / Prefix / Suffix の語彙リスト | Phase 5 着手前 |
| P4 | 残り経済メトリクス数式(cash_coverage_ratio / buyback_liability_ratio / forecasted_cash_coverage) | Phase 9 着手前 |
| P5 | 残りストレスシナリオ(Base / High Survival / Low Burn / P2P Freeze / Buff Overpower)のパラメータと合格基準 | Phase 9 着手前 |
| P6 | Memorial NFTのチェーン・規格の最終確認(既定: Polygon PoS / ERC-721) | Phase 7 着手前 |
| P7 | 「LUCKトレーニング」の定義 — トレーニング種別は SPEED/POWER/RECOVERY の3種のみだが、仕様03に「LUCK型が有効なLUCKトレーニングを受けた場合 random_modifier 範囲が変化」とある。LUCK型が任意のトレーニングを受けた場合を指すのか、第4の種別なのか | Phase 5 着手前 |

### E-3. 未確定(オーナー判断待ち)

| # | 未確定事項 | 詳細 | ブロックするフェーズ |
|---|---|---|---|
| E14 | 大口出金のAdmin Review閾値 | 「large withdrawals may require Admin Review」の金額基準とフロー | Phase 12 |
