# ログイン・アカウント連携 記録(Decision 071 / 072 / 083)

> 2026-07-04 / オーナー決定・実装・本番稼働確認済み。ログイン周りの正典。
> 関連: Decision Log 071(ログイン方式)・072(相互連携)・083(Google一本化)/ コミット 9286e91・4b666ee・65a942e

## 0. 【現行】ログインはGoogleのみ(Decision 083, 2026-07-11)

- **提供するログインはGoogle OAuthのみ**。`/login`ページは削除し、トップページの各CTAが
  直接Google OAuthを開始する(黒のGoogle公式スタイルボタン。ヒーローのボタンだけは
  デザイン変更禁止のため見た目従来どおり・動作のみGoogle直起動)。
- 未認証アクセスは `/login` ではなく `/` へリダイレクト(`serverApiOrLogin`)。
- 撤去理由: 本サイトはカストディアルでdappsではない(ユーザーがトランザクションに署名する
  場面がゼロ。記念NFTのミント先もカストディウォレット)。MetaMaskログインは純粋な署名認証
  でしかなく、しかもウォレット単独アカウントは合成メール `{uuid}@user.sevendays` になり
  ウェルカム/CSメール不達・ギフト受取不可の欠陥アカウントだった。メール+パスワードも同時撤去。
- **UIのみの変更で可逆**: SupabaseのWeb3/メールプロバイダーと`user_wallets`は無効化していない。
  /accountのウォレット紐づけ(Decision 072)は「将来の記念NFT引き出し先アドレス」として存続。
- 既存のMetaMask/メール登録アカウント(オーナーの管理者アカウント=メール方式を含む)は、
  同一メールのGoogleログインで自動マージ(§1実測済み)またはサポート対応で救済。
  以下§1〜2は歴史的経緯として残す。

## 1. 提供するログイン方式(Decision 071・→083でGoogleのみに縮小)
