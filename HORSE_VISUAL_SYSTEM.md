...
2. **副色 coatB**(2色目)= coat から関係性を rng で選択。**地味を減らしハッキリ違う配色を多く**:
   | 分岐 | 確率 | 内容 |
   |---|---|---|
...
- `Landing.tsx` は `pickShowcase(8, () => (Math.random()*0xffffffff)>>>0)`。8頭=8系統が虹状に配置され、縦横どの隣も系統が大きく違う。ヒーローは固定金色 `base_01` + `upperLower` の淡い上下2色。
