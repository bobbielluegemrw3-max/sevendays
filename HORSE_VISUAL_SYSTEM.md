...
> **★2026-07-06 全面移行: 「3アーキタイプ方式」が現行正典**(§7.0)。§7.1〜7.4の
> 「18素体×手続き着色」方式は旧方式(コードは残存するが馬の描画には不使用)。

### 7.0 3アーキタイプ方式(現行 — 2026-07-06 オーナー決定)

- **経緯**: グレースケール素体への手続き着色は品質天井が低い(単色化・金属感の喪失・配色事故)と
  オーナーが判断。Manusフルカラー原画3枚(V2金×黒 / V3虹色クローム / V4黒メカ)を品質基準として採用し、
  ポーズ量産(18素体・batch03R)を**中止**。
- **仕組み**: `馬の見た目 = アーキタイプ(3) × ボディ配色(12) × たてがみ(16)` = **576ルック**。
  全バリエーションはシートA(ボディ=色相回転12角度)/シートB(たてがみ=回転12+単色4)で**オーナー承認済み(全承認)**。
  エンジンは色を作らず、承認リストから dnaHash で決定論的に**選ぶだけ**。
- **アセット**: Manus納品のレイヤー分離原画(排他マスク・再合成でピクセル完全一致を検収済み)。
  web用: `apps/web/public/horses/nft/{v2,v3,v4}_{coat,mane_tail,accents,eye_glow}.png`(768px)。
  2048px原本はスクラッチパッド`nftlayers/`+ZIP `nft_v2v3v4_layers.zip`。
- **コード**: `apps/web/lib/nft-visual.ts`(`deriveNftLook(dnaHash, name)` / `pickNftShowcase`)+
  `apps/web/components/NftHorseArt.tsx`(真HSVの色相変換でcoat/maneを個別変換、accents金と目は固定)。
  名前Prefix→たてがみ色ターゲット(`PREFIX_TARGET`、Silver/White=銀白)+アーキタイプbias(`ARCH_BIAS`)。
  CSS filterの行列近似ではなく**シート検収と同じ真HSV**を使用(承認見た目の厳密再現)。
- **利用箇所**: ランディングのマケプレ(`pickNftShowcase`)・ダッシュボード/厩舎/馬詳細(`deriveNftLook`)。
  ヒーローはV2原画そのまま(`/horses/manus/v2.png`)。
- **拡張**: レア度演出(細部差分版)はManusへの追加発注で対応可。ボディ/たてがみの承認リストは
  `nft-visual.ts` の `BODY_DEGS` / `MANE_VARIANTS` を編集するだけ。

---

### (旧方式 — 2026-07-05 まで) 18素体×手続き着色

> §2〜3は当初の発注/設計ドラフト。以下は旧実装の記録。コードは
> `apps/web/lib/horse-visual.ts`(名前/レア度/価格導出は現在も使用)と
> `apps/web/components/HorseArt.tsx`。
