# 馬ベースアート 全面改修 引継ぎ書 【役目終了 — 2026-07-06】

> **★このドキュメントは歴史的記録。** ポーズ量産方式(18素体+batch03R)はオーナー判断で中止され、
> **「3アーキタイプ方式」に全面転換**した。現行の正典は `NFT_ART_HANDOVER.md` と `HORSE_VISUAL_SYSTEM.md §7.0`。
...
> **進捗更新(2026-07-05 夜)**: batch01R(base_01〜12)・batch02R(base_13〜24)を中間検収**合格**
> (独立検証: グレースケール0%・不透明率整合・mane_tail背景バグなし・マージン最小47px。検証スクリプトは
> スクラッチパッド `batch01R/verify2.py`)。オーナー除外指定=**02,05,13,14,18,23** → **採用18体**。
> 手順④⑤実施済み: 旧31体を全削除し、新18体(512px)+`bases.json`+`horse-visual.ts` の `BASES` を差し替え。
> lint/build/バンドル検査PASS・TOP目視OK。ヒーローは**base_24**(オーナー指定)。
> 追加改修(同日): **純色モード**(~24%が全身1色の原色/真っ黒/真っ白)+ **pickShowcase見た目分離**
> (色相36°未満却下・配色ペア却下・dark/pale各1頭上限)。詳細は `HORSE_VISUAL_SYSTEM.md §7.1(1b)/§7.3(1b)`。
> **ここまでコミット済み(オーナー承認 2026-07-05)**。
> 一覧ページは `horse-bases-sheet-v3.html`(24体全量は `public/horses/bases_v3/` に退避、gitignore済み)。
> **残: batch03R(base_25〜36)の納品→検収→除外選定→追加**(希少ポーズを入れるかは未決)。
